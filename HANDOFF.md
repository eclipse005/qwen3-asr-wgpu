...
## CPU 后端的性能账（2026-09-15 第四轮，全部实测）

**已落地**（每条都过 6/6 CPU 门禁，180s_en：83.6 s/2.11× → **65.1 s/2.71×**）：

| 修的病理 | 数字 |
|---|---|
| GEMM 内积单累加器（4 周期延迟链） | 15s prefill 5.4 → 1.36 s |
| `f16→f32` 留在 GEMM 内循环（每元素被转 `n_out` 次） | 转换受限 125 MB/s → 解除 |
| prefill 注意力逐 key 转 f16 cache | 34 s prefill 里 18 → 2.9 s |
| 注意力内积单累加器 | 147 → 63 ms/token |
| 注意力按「行」并行（decode 只有 1 行 ⇒ 单核） | decode 136 → 36.7 s |

**测得但没采纳（别再试）**：

1. **KV cache 存 f16 + 每层展开成 f32**：展开的时候**用了连续行下标**，而 cache 的行距是
   `max_seq`（不是 `cur`）⇒ 读到别的位置，输出变垃圾、还更慢。修好后（按 head 正确展开）
   它在 prefill 上是 5× 的好，但 decode 每 token 要重展 19 MB/层 ⇒ 净亏。
   **现在的做法**：cache 直接存 f32（值是 f16 舍入后的原值，等价），读写都不再转换。
2. **权重存 f16 + F16C(AVX2) 一次转 8 个**：decode −9%（36.7→33.4 s）但 **prefill +28%**
   （20.2→25.8 s）——把整行权重写进 f32 scratch 的流量吃掉了 f16 读省下的钱。净亏，回退。
